...
class InvoiceSDK
{
    ...
    public function sendPostSyncRequest(string $senId, string $token, string $method, mixed $content, int $timeOut = 6): mixed
    {
        ...
        try {
            ...
            $sign = self::makeSign($urlInfo['path'], $appSecret, $appKey, $senId, (string) $nonce, $content, (string) $timestamp);
    ...
    public function getBody(array $params, string $method): bool|string
    {
        ...
        switch ($method) {
            ...
            case 'nuonuo.OpeMplatform.requestBillingNew':
                $body = [
                    'order' => [
                        ...
                        'invoiceNumEnd' => $params['invoiceNumEnd'] ?? '',
            ...
            case 'nuonuo.OpeMplatform.saveInvoiceRedConfirm':
                $body['billId']                = $params['billId']                ?? '';
                $body['blueInvoiceLine']       = $params['blueInvoiceLine']       ?? '';
                $body['applySource']           = $params['applySource']           ?? '';
                $body['blueInvoiceNumber']     = $params['blueInvoiceNumber']     ?? '';
                $body['blueInvoiceCode']       = $params['blueInvoiceCode']       ?? '';
                $body['blueElecInvoiceNumber'] = $params['blueElecInvoiceNumber'] ?? '';
                $body['billTime']              = $params['billTime']              ?? '';
                $body['sellerTaxNo']           = self::$config['tax_num']         ?? '';
                $body['sellerName']            = $params['sellerName']            ?? '';
                $body['departmentId']          = $params['departmentId']          ?? '';
                $body['clerkId']               = $params['clerkId']               ?? '';
                $body['buyerTaxNo']            = $params['buyerTaxNo']            ?? '';
                $body['buyerName']             = $params['buyerName']             ?? '';
                $body['vatUsage']              = $params['vatUsage']              ?? '';
                $body['saleTaxUsage']          = $params['saleTaxUsage']          ?? '';
                $body['accountStatus']         = $params['accountStatus']         ?? '';
                $body['redReason']             = $params['redReason']             ?? '';
                $body['extensionNumber']       = $params['extensionNumber']       ?? '';
                $body['autoInvoice']           = $params['autoInvoice']           ?? '';
                $body['orderNo']               = $params['orderNo']               ?? '';
                $body['phone']                 = $params['phone']                 ?? '';
                $body['email']                 = $params['email']                 ?? '';
                $body['callbackUrl']           = self::$config['call_back_url']   ?? '';
                if (!empty($params['detail'])) {
                    $body['detail']['blueDetailIndex']    = $params['detail']['blueDetailIndex']    ?? '';
                    $body['detail']['goodsName']          = $params['detail']['goodsName']          ?? '';
                    $body['detail']['unit']               = $params['detail']['unit']               ?? '';
                    $body['detail']['specType']           = $params['detail']['specType']           ?? '';
                    $body['detail']['num']                = $params['detail']['num']                ?? '';
                    $body['detail']['taxExcludedPrice']   = $params['detail']['taxExcludedPrice']   ?? '';
                    $body['detail']['taxExcludedAmount']  = $params['detail']['taxExcludedAmount']  ?? '';
                    $body['detail']['taxAmount']          = $params['detail']['taxAmount']          ?? '';
                    $body['detail']['taxRate']            = $params['detail']['taxRate']            ?? '';
                    $body['detail']['goodsCode']          = $params['detail']['goodsCode']          ?? '';
                    $body['detail']['favouredPolicyFlag'] = $params['detail']['favouredPolicyFlag'] ?? '';
                }

                break;

            case 'nuonuo.OpeMplatform.queryInvoiceRedConfirm':
                $body['identity']    = $params['identity']    ?? '';
                $body['billStatus']  = $params['billStatus']  ?? '';
                $body['billId']      = $params['billId']      ?? '';
                $body['billNo']      = $params['billNo']      ?? '';
                $body['billUuid']    = $params['billUuid']    ?? '';
                $body['billTimeEnd'] = $params['billTimeEnd'] ?? '';
                $body['pageSize']    = $params['pageSize']    ?? '';
                $body['pageNo']      = $params['pageNo']      ?? '';

                break;

            case 'nuonuo.OpeMplatform.fastInvoiceRed':
                $body['orderNo']         = $params['orderNo']             ?? '';
                $body['extensionNumber'] = $params['extensionNumber']     ?? '';
                $body['clerkId']         = $params['clerkId']             ?? '';
                $body['deptId']          = $params['deptId']              ?? '';
                $body['orderTime']       = $params['orderTime']           ?? '';
                $body['taxNum']          = self::$config['tax_num']       ?? '';
                $body['invoiceCode']     = $params['invoiceCode']         ?? '';
